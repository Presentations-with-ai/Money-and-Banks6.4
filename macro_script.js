...
function applyTestModeMenuState(mode) {
  ...
  if (modeHint) {
    modeHint.textContent = isSpeedTestMode(normalizedMode)
      ? 'Режим на скорость: таймер не ограничивает ответы, секундомер идёт вверх'
      : 'Обычный режим: на каждый вопрос действует таймер';
  }
}
...
<div class="card">
    ...
    <div class="mode-hint" id="test-mode-hint">Обычный режим: на каждый вопрос действует таймер</div>
